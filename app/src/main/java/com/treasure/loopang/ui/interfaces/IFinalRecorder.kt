...
import com.treasure.loopang.audio.SoundRange
...
/*    fun onPlayStart()              // 재생시작일 때
    ...
    fun setEffectToBlock(layerId: Int, blockId: Int, effect: EffectorPresets)   // 블록에 이펙트 적용(레이어아이디, 블록아이디, 이펙트프리셋) // 이펙트는 형식이 뭘까...
    ...
    fun getBlockList(): List<List<SoundRange>>

    fun getBlockList(layerId: Int): List<SoundRange> // 블록리스트 레이어로 들고오기

    fun insertSounds(soundList: List<Sound>) // 믹서에 사운드 넣기
